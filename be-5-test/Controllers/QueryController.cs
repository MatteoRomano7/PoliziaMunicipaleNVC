using be_5_test.Models;
using Microsoft.AspNetCore.Mvc;
using Microsoft.Data.SqlClient;

namespace be_5_test.Controllers
{
    public class QueryController : Controller
    {
        ...
        public IActionResult Punti()
        {
            ...
            {
                DB.conn.Open();
                var cmd = new SqlCommand("SELECT a.Cognome,a.Nome, v.IndirizzoViolazione, v.DataViolazione,  v.Importo, v.DecurtamentoPunti  FROM [dbo].[Verbale] as v  join dbo.Anagrafica as a on a.IDAnagrafica = v.IDAnagrafica  WHERE v.DecurtamentoPunti > 10", DB.conn);
                var reader = cmd.ExecuteReader();
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        var verbaleTot = new PuntiSaldo()
                        {
                            Cognome = reader["Cognome"].ToString(),
                            Nome = reader["Nome"].ToString(),
        ...
        public IActionResult FiltroImporto()
        {
            ...
            {
                DB.conn.Open();
                var cmd = new SqlCommand("SELECT a.Cognome, a.Nome, v.DataViolazione,  v.Importo, v.DecurtamentoPunti  FROM [dbo].[Verbale] as v  join dbo.Anagrafica as a on a.IDAnagrafica = v.IDAnagrafica  WHERE v.Importo > 400", DB.conn);
                var reader = cmd.ExecuteReader();
                if (reader.HasRows)
                {
                    while (reader.Read())
